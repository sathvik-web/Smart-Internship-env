...
def compute_reward(action: Action, task: InternshipTask, progress_ratio: float) -> Reward:
	"""
	Compute deterministic reward in [0, 1].

	Components:
	- decision correctness
	- closeness to true relevance score
	- reasoning quality via expected keywords
	- ranking quality for hard tasks
	- progress bonus
	- explicit penalty for incorrect high-confidence decisions
	"""
	decision_score = 1.0 if action.decision == task.correct_decision else 0.0
	score_closeness = _clamp(1.0 - abs(action.relevance_score - task.true_score))
	reasoning_score = _reasoning_keyword_score(action.reasoning, task.expected_reasoning_keywords)
	ranking_score = _ranking_score(action.ranking, task.correct_ranking)
	...
	if is_hard:
		...
		weighted_core = 0.45 * decision_score + 0.35 * score_closeness + 0.20 * reasoning_score

	progress_bonus = 0.08 * _clamp(progress_ratio)

	penalty = 0.0
	if action.decision != task.correct_decision:
		confidence_gap = abs(action.relevance_score - 0.5)
		penalty += 0.10 + 0.10 * confidence_gap

	# Penalize high absolute score error to discourage overconfident bad calibration.
	score_error = abs(action.relevance_score - task.true_score)
	if score_error > 0.45:
		penalty += 0.12
	elif score_error > 0.30:
		penalty += 0.06

	# Reasoning quality should matter beyond weighted contribution.
	if reasoning_score < 0.20:
		penalty += 0.08
	elif reasoning_score < 0.40:
		penalty += 0.04

	if task.correct_ranking and not action.ranking:
		penalty += 0.08
